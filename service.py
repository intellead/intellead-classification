...
def classification(lead):
    ...
    if ('main_activity' in lead.keys()):
        knn = KNeighborsClassifier(n_neighbors=16, p=2)
        knn.fit(inputs_training, outputs_training)
        print('[K=16] The probability of the algorithm to be right is: %f%%' % (
        knn.score(inputs_test, outputs_test) * 100))
    else:
        knn = KNeighborsClassifier(n_neighbors=7, p=2)
        knn.fit(inputs_training, outputs_training)
        print('[K=7] The probability of the algorithm to be right is: %f%%' % (
        knn.score(inputs_test, outputs_test) * 100))
    print('Lead data:')
    print(lead)
